...
export class HomePage extends BasePage {
    ...
    async selectRandomResult() {
        ...
        await expect.poll( async()=>{ return await this.vehicleNames.count() }).toBeLessThan(10);

        const count = await this.vehicleNames.count();
        const randomIndex = Math.floor(Math.random() * count);
        console.log(`Clicking random element at index: ${randomIndex} of ${count}`);

        const selectedVehicle = await this.vehicleNames.nth( randomIndex ).textContent();
